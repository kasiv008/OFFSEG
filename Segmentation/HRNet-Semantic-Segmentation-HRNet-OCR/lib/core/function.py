...
def test(config, test_dataset, testloader, model,
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(testloader)):
            ...
            b,_,_,_ = pred.shape
            for i in range(b):
                #sv_path = '/media/moonlab/New Volume/RELLIS-3D/benchmarks/HRNet-Semantic-Segmentation-HRNet-OCR/pred'#os.path.join(config.OUTPUT_DIR, 'hrnet',name[i][:5],'pylon_camera_node_label_id')
                #if not os.path.exists(sv_path):
                   # os.makedirs(sv_path)
